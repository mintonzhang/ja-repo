...
public class HttpRemoteFetcher {
  ...
  public HttpRemoteFetcher(OutboundRequestPolicy outboundPolicy, NexusPlusMetrics metrics) {
    this(
        outboundPolicy,
        metrics,
        "HTTP_1_1",
  ...
  public HttpRemoteFetcher(
      ...
      @Value("${nexus-plus.proxy.remote-http-version:HTTP_1_1}") String remoteHttpVersion,
      @Value("${nexus-plus.proxy.search-timeout-seconds:30}") long searchTimeoutSeconds,
      @Value("${nexus-plus.proxy.metadata-timeout-seconds:60}") long metadataTimeoutSeconds,
      @Value("${nexus-plus.proxy.content-timeout-seconds:${nexus-plus.proxy.get-timeout-seconds:120}}") long contentTimeoutSeconds,
      @Value("${nexus-plus.proxy.head-timeout-seconds:5}") long headTimeoutSeconds,
      @Value("${nexus-plus.proxy.body-read-retry-attempts:1}") int bodyReadRetryAttempts) {
  ...
  private Result fetchInternal(Request req, int redirects) throws IOException {
    ...
    HttpRequest.Builder b = HttpRequest.newBuilder()
        ...
        .header("Accept", "*/*")
        .header("User-Agent", "nexus-plus/0.1");
