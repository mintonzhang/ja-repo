...
This document covers local startup, release builds, archive package deployment, production deployment architecture, and resource sizing for kkrepo. Hot-reload development and protocol testing are documented in the [Development Guide](development-guide.md).
...
For a quick local trial, start kkrepo and MySQL with the public release image:
...
The `.env` encryption secrets generated by the script are only for local trials. For externally reachable deployments, replace `KKREPO_CREDENTIAL_SECRET` and `KKREPO_API_KEY_PAYLOAD_SECRET`, and use an independent MySQL instance plus OSS/S3 blob storage.
...
The default configuration connects to `127.0.0.1:13306/kkrepo`. If your local MySQL uses a different port or account, override it with Spring Boot environment variables:
...
The startup script uses the extracted directory as `KKREPO_HOME` by default. Common paths can be overridden with environment variables:

| Environment variable | Default | Description |
| --- | --- | --- |
| `KKREPO_HOME` | Extracted directory | Application home |
...
- kkrepo service instance: at least 2C4G per production instance. For high availability, deploy at least 2 replicas and scale horizontally based on request volume.
