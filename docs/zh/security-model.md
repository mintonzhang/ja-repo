...
本文说明 nexus-plus 当前安全模型：认证、授权、密钥、审计日志和运维边界。
...
nexus-plus 使用 Nexus 风格 privilege。仓库权限动作包括：
...
nexus-plus 将 API-key 兼容数据存储在 MySQL 中。用户可见原始 token 以加密 payload 保护，查找使用 hash 材料而不是明文 token。
...
`NEXUS_PLUS_CREDENTIAL_SECRET` 保护可复用凭据，包括：

- Blob-store S3/OSS key。
- LDAP bind password。
- OIDC client secret。

`NEXUS_PLUS_API_KEY_PAYLOAD_SECRET` 保护用户可见 API-key payload。
