...
nexus-plus 是一个兼容 Nexus 的制品仓库，核心设计是 MySQL-first 元数据、OSS/S3-first blob 存储，以及适合多副本部署的运行时行为。

项目目标不是复制 Nexus 内部实现。nexus-plus 保持客户端可见协议、权限模型和 `/repository/<repo>/...` URL 兼容，同时避免在生产运行时依赖 OrientDB、内嵌 Elasticsearch、Karaf、OSGi 和只能本地持久化的 blob 状态。
...
缓存失效依赖 TTL 和 MySQL version 水位。运维人员可以通过 `NEXUS_PLUS_*` 环境变量调整 cache 大小和 TTL。
...
除非用户可见兼容性需要适配，nexus-plus 不主动复制以下 Nexus 内部机制：
