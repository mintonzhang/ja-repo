...
本文说明从 Nexus Repository 迁移到 nexus-plus 的准备条件、执行顺序、增量迁移和域名切换方式。

nexus-plus 兼容 Nexus 的 `/repository/<repo>/...` URL 布局、客户端协议行为和权限认证模型。迁移完成后，只需要把原 Nexus 域名指向 nexus-plus，Maven、npm、PyPI、Go、Helm、NuGet、RubyGems、Yum 等客户端配置不需要修改。
...
| 顺序 | 后台入口 | 操作 | 结果 |
| --- | --- | --- | --- |
| 1 | 源 Nexus | 开启 Script REST API 脚本创建能力 | nexus-plus 可以读取普通 REST API 无法直接提供的源端数据 |
...
| 4 | Nexus Repository Data | 执行 `Sync packages` | 迁移真实 blob/package 数据到 nexus-plus |
| 5 | Nexus Repository Data | 上线前按需重复增量 `Sync metadata` 和 `Sync packages` | 迁移切换窗口内新增或更新的数据 |
| 6 | DNS 或反向代理 | 把原 Nexus 域名指向 nexus-plus | 客户端无感切换到 nexus-plus |

## 迁移前准备

- 确认 nexus-plus 已经部署并连接生产 MySQL。
- 在 nexus-plus 中先创建名为 `default` 的 blob store；生产环境建议使用 OSS/S3 blob store。
...
先进入 nexus-plus 后台 `/admin/` 的 `Nexus Metadata` 页面，填写源 Nexus 地址、用户名、密码和版本。
...
仓库数据迁移在 nexus-plus 后台 `/admin/` 的 `Nexus Repository Data` 页面执行，分两步：

1. 先迁移仓库元数据：扫描源 Nexus hosted 仓库中的 component、asset、路径、大小、content-type、时间戳和 blob 引用等信息，在 nexus-plus MySQL 中生成迁移任务。
2. 再迁移 blob 真实数据：按迁移任务下载源 Nexus asset 内容，并写入 nexus-plus 的目标 blob store。
...
迁移任务、仓库扫描游标、asset 状态和失败信息都存储在 MySQL 中。迁移过程中如果 nexus-plus 重启、网络中断、源 Nexus 短暂不可用或页面关闭，可以继续执行：
...
确认无问题后，把原 Nexus 域名通过 DNS 或反向代理指向 nexus-plus。由于 nexus-plus 兼容 Nexus 的 `/repository/<repo>/...` URL 布局、客户端协议和权限认证模型，客户端不需要修改 Maven settings、npm registry、PyPI index-url、Go GOPROXY、Helm repo 等配置。

切换完成后，建议保留源 Nexus 一段观察期，并在确认不再需要补偿迁移后关闭源端脚本能力。

## 源端脚本能力配置

从 Nexus 迁移到 nexus-plus 前，源端 Nexus 需要允许通过 Script REST API 创建并执行临时 Groovy 脚本。

nexus-plus 迁移流程会在源 Nexus 端临时创建脚本，用于补偿 Nexus 普通 REST API 无法直接读取的数据，例如本地用户 password hash、API key 兼容信息，以及 hosted 仓库资产分页发现。脚本执行完成后，nexus-plus 会尝试删除临时脚本。
...
也可以直接在 nexus-plus 后台管理页面执行 Nexus Metadata preflight。若脚本能力未开启，源端通常会在创建脚本时返回 `410 Gone`，并提示创建或更新脚本已被禁用。
...
2. 重启源 Nexus。
3. 确认没有残留 `nexus-plus-*` 临时脚本；如有残留，可通过 Script REST API 删除。
